...
/*
 * Content-script side — the original feature: translate what is selected.
 *
 * Selection capture, the non-destructive overlay, and replace-in-place for
 * editable inputs. The model call is delegated to the background worker.
 *
 * Lived in entrypoints/content.ts until that became the composition root for
 * every page-facing module; the behaviour is unchanged.
 */
...
/** Show a short status line near the selection — used by page-level actions. */
export function showNotice(message: string) {
  const box = baseBox();
  box.textContent = message;
  setTimeout(() => {
    if (activeOverlay === box) removeOverlay();
  }, 2500);
}
...
/** Translate the current selection and show (or insert) the result. */
