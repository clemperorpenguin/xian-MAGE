...
"""The machinery both live engines share.

``live_lens`` (one vision call does everything) and ``live_ocr`` (a local
reader, then a translation call) differ only in how one frame becomes a list of
translated regions.  Everything around that — the tick, the change gate, the
carry, the masking, the publishing — was earned against real failures and there
must only be one copy of it:

* **A change gate.** Re-running inference on an unchanged frame is pure waste,
  so a perceptual hash of the raw crop decides whether anything actually moved.
  It is a distance threshold, not the exact-match cache the bubble path uses,
  because anti-aliasing and cursor blink perturb a few bits constantly.
...
A subclass supplies :meth:`LiveWorkerBase._translate_frame` and nothing else.
"""
...
class LiveWorkerBase(QThread):
    ...
    # ── the engine-specific half ─────────────────────────────────────

    async def _translate_frame(self, frame: Image.Image):
        """Produce located, translated regions for one frame.

        Return a list of objects carrying ``box``, ``original``, ``translated``
        and ``is_valid()`` — :class:`xian.grounding.TextRegion` is one — or
        ``None`` to leave the overlay as it is.  ``None`` covers both "the call
        failed" and "nothing on screen has changed", which want identical
        handling: keep carrying what is painted.
        """
